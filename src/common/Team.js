...
import { 
  ...
  StatusBar, TouchableOpacity, Modal, Pressable, ActivityIndicator 
} from 'react-native';
import FontAwesome6 from 'react-native-vector-icons/FontAwesome6';
import LinearGradient from 'react-native-linear-gradient';

const SmartImage = ({ uri, style, iconSize = 20 }) => {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);

  return (
    <View style={[style, styles.imageCenterer]}>
      {/* The Actual Image */}
      <Image
        source={{ uri: uri }}
      ...
      {error && (
        <FontAwesome6 name="user-tie" size={iconSize} color="rgba(255,255,255,0.5)" />
      )}
    </View>
  );
};
  ...
  const sections = [
    {
      ...
      hasImage: true,
      imageUri: 'https://mguvv.ac.in/angular/assets/vcphoto/vc_1.jpeg',
    ...
    {
      title: 'Nodal Officer',
      name: 'Dr. Toran Lal Sahu', 
      role: "Assistant Professor/ HOD",
      hasImage: true,
      imageUri: 'https://mguvv.ac.in/__Files/EmployeeDetail/1025/_ProfilePic_20250422013710722.jpeg',
    ...
    },
    {
      ...
      details: "Technical development and maintenance support provided by National Informatics Centre (NIC)."
    },
  ];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topHeader}>
        <Text style={styles.title}>University Team</Text>
        <Text style={styles.subTitle}>MGUVV Leadership & Support</Text>
      </View>
            ...
            {selectedMember && (
              <View style={styles.modalBody}>
                {selectedMember.hasImage ? (
                  <SmartImage uri={selectedMember.imageUri} style={styles.modalPortrait} iconSize={40} />
                ) : (
                    ...
                    {item.isGroup ? (
                      <View style={styles.avatarStack}>
                        {item.teamAvatars.map((url, i) => (
                          <SmartImage key={i} uri={url} style={[styles.stackImg, { marginLeft: i === 0 ? 0 : -12 }]} />
                        ))}
                      </View>
                    ) : (
                      <SmartImage uri={item.imageUri} style={styles.avatar} />
                    )}
...
const styles = StyleSheet.create({
  ...
  topHeader: { paddingHorizontal: 20, paddingVertical: 20, alignItems: 'center', backgroundColor: '#FFF', borderBottomLeftRadius: 30, borderBottomRightRadius: 30, elevation: 3, marginBottom: 15 },
  title: { fontSize: 22, fontWeight: '900', color: '#1A1D2E' },
  subTitle: { fontSize: 13, color: '#666', marginTop: 2 },
  sectionLabel: { fontSize: 13, fontWeight: 'bold', color: '#888', marginBottom: 8, paddingHorizontal: 25, textTransform: 'uppercase' },
  cardWrapper: { marginBottom: 15 },
  mainCard: { borderRadius: 22, marginHorizontal: 20, overflow: 'hidden', elevation: 4 },
  cardOverlay: { padding: 18, backgroundColor: 'rgba(255, 255, 255, 0.1)' },
  row: { flexDirection: 'row', alignItems: 'center' },
  imageCenterer: { justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.05)' },
  ...
  memberRole: { color: 'rgba(255, 255, 255, 0.9)', fontSize: 12, marginTop: 2 },
  ...
  modalPortrait: { width: 120, height: 120, borderRadius: 60, marginBottom: 15, borderWidth: 4, borderColor: '#F4F7FF' },
  ...
  modalName: { fontSize: 20, fontWeight: '800', color: '#1A1D2E', textAlign: 'center' },
  modalRoleText: { fontSize: 14, color: '#666', marginTop: 4, textAlign: 'center' },
  ...
  modalBio: { fontSize: 14, color: '#444', textAlign: 'center', lineHeight: 20, marginTop: 15 }
});
...
//       title: 'Nodal Officer',
//       name: 'Dr. Toran Lal Sahu', 
//       role: "Assistant Professor/ Head of Department College of Horticulture, Hatani",
//       hasImage: true,
//       imageUri: 'https://mguvv.ac.in/__Files/EmployeeDetail/1025/_ProfilePic_20250422013710722.jpeg',
//       colors: ['#FF8008', '#FFC837'],
//       details: "Coordinating digital initiatives and technical implementation across university departments."
